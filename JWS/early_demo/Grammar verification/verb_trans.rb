...
def mei(line, spos)
  if /[a-zA-Z]/ !~ line.split("「")[1].chop
    puts "[#{line.split("「")[0]}][#{spos}]\n" + "[#{line.split("「")[1].chop}][#{spos}]\n"
  else
    puts "[#{line.split("「")[0]}][#{spos}]\n"
  end
end
...
def masu(line, spos)
  ...
  if spos[-1]=="１"
    puts "[#{line.split("「")[0]}][#{line.split("「")[0].chop+$gojyuuon[line.split("「")[0][-1].to_sym][1]+"ます"}][#{spos}][ます形][现在时肯定形]\n" + "[#{line.split("「")[1].chop}][#{line.split("「")[0].chop+$gojyuuon[line.split("「")[0][-1].to_sym][1]+"ます"}][#{spos}][ます形][现在时肯定形]\n"
  elsif spos[-1]=="２"
    puts "2"
  else
    puts "3"
...
#  gojyuuon = {う: %w(わ い え お), く: %w(か き け こ), ぐ: %w(が ぎ げ ご), す: %w(さ し せ そ), つ: %w(た ち て と), ぬ: %w(な に ね の), ぶ: %w(ば び べ ぼ), む: %w(ま み め も), る: %w(ら り れ ろ)}
  ...
  if spos[-1]=="１"
    ...
    "[#{jishokei_a}][#{masukei}][#{spos}][ます形][现在时肯定形]\n" + "[#{jishokei_b}][#{kana_masukei}][#{spos}][ます形][现在时肯定形]\n"
  elsif spos[-1]=="２"
    ...
    if jishokei_a=="来る"
      ...
      if /[a-zA-Z]/ =~ jishokei_b
        ...
        "[#{jishokei_a}する][#{masukei}][#{spos}][ます形][现在时肯定形]\n"
      else
        if jishokei_a[-2]+jishokei_b[-1]!="する"
          masukei = jishokei_a+"します"
        ...
        "[#{jishokei_a}する][#{masukei}][#{spos}][ます形][现在时肯定形]\n" + "[#{jishokei_b}する][#{kana_masukei}][#{spos}][ます形][现在时肯定形]\n"
      end
    end
  end
end
